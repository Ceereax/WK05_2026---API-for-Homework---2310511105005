...
async function findOne(dbname, colname, filter) {
  const dbconn = await connect();
  const collection = getCollection(dbconn, dbname, colname);
...
async function updateOne(dbname, colname, filter, updateobj) {
  const dbconn = await connect();
  const collection = getCollection(dbconn, dbname, colname);
...
module.exports = { remove, find, findOne, insert, update, updateOne };